...
contract NFTAuction {
    ...
    ERC20 token = ERC20(0x7f629f02e0E9529887146d04efa633f2219Bb5b4);  // Testnet
    ERC2981 royalty = ERC2981(0x3feBAB631305BBCE5ab4Eb2E5cf297008Fadb1e1); // NFT contract testnet
    ...
    struct Auction {
        ...
        uint tokenBuyNowPrice; // token price
        uint nftHighestBid;
    ...
    function isAuctionOngoing(address nftContractAddress, uint tokenId) internal view returns(bool) {
        uint auctionEndTimestamp = nftContractAuctions[nftContractAddress][tokenId].bidPeriod;
    ...
    */
    ...
    /**
        SALES
    */
    function setupSale(address nftContractAddress, uint tokenId, uint _buyNowPrice, uint tokenPrice) internal {
        Auction storage auctions = nftContractAuctions[nftContractAddress][tokenId];

        auctions.buyNowPrice = _buyNowPrice;
        auctions.nftSeller = msg.sender;
        auctions.tokenBuyNowPrice = tokenPrice;

        IERC721(nftContractAddress).transferFrom(msg.sender, address(this), tokenId);
    }

    function createSale(address nftContractAddress, uint tokenId, uint _buyNowPrice, uint tokenPrice) external isAuctionNotStartedByOwner(nftContractAddress, tokenId) priceGreaterThanZero(_buyNowPrice) {
        setupSale(nftContractAddress,tokenId,_buyNowPrice, tokenPrice);
    ...
    function buyNow(address nftContractAddress, uint tokenId) external payable notNftSeller(nftContractAddress, tokenId) returns(bool) {
        ...
        bool sent = payable(seller).send(sellerPayout);
    ...
    function buyNowWithToken(address nftContractAddress, uint tokenId, uint tokenAmount) external notNftSeller(nftContractAddress, tokenId) returns(bool) {
        Auction storage auctions = nftContractAuctions[nftContractAddress][tokenId];
        ...
        // Check that it is on sale
        require(buyNowPrice != 0, "Cannot buy with token");
        ...
        // Transferring tokens
        token.transferFrom(msg.sender, seller, sellerPayout);

        token.transferFrom(msg.sender, commissionAddress, comm);
    ...
    // Call approve of token before calling this function
    function makeBidWithToken(address nftContractAddress, uint tokenId, uint tokenAmount) external auctionOngoing(nftContractAddress, tokenId) {
        address nftPrevHighestBidder = nftContractAuctions[nftContractAddress][tokenId].nftHighestBidder;
        uint nftPrevHighestBid = nftContractAuctions[nftContractAddress][tokenId].nftHighestBid;
        ...
        if(nftPrevHighestBidder != address(0)){
            token.transfer(nftPrevHighestBidder, nftPrevHighestBid);
        }
    }
    /**
        BID FUNCTIONS END
